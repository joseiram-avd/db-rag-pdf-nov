...
# MAGIC %pip install --quiet -U transformers==4.41.1 pypdf==4.1.0 langchain-text-splitters==0.2.0 databricks-vectorsearch mlflow tiktoken==0.7.0 torch==2.3.0 llama-index==0.10.43 markdownify 
# MAGIC
...
# DBTITLE 1,Create Directory in Specified Volume Folder
if not dbutils.fs.ls(f"{volume_folder}"):
    dbutils.fs.mkdirs(f"{volume_folder}")

# COMMAND ----------

# DBTITLE 1,Create and Print Folder Volume Path
...
# DBTITLE 1,Upload Sample PDFs for Testing
...
# DBTITLE 1,List Files in Specified Folder Path
...
(df.writeStream
  ...
  .option("checkpointLocation", f'dbfs:{volume_folder}/checkpoints/invoices')
  .table('invoices').awaitTermination())

# COMMAND ----------

# MAGIC %sql SELECT * FROM invoices
...
# DBTITLE 1,Convert PDF Bytes to Markdown Text
import markdownify

def convert_pdf_to_markdown(raw_doc_contents_bytes: bytes):
    text = parse_bytes_pypdf(raw_doc_contents_bytes)
    if text is None:
        return None
...
with requests.get('https://github.com/databricks-demos/dbdemos-dataset/blob/main/llm/databricks-pdf-documentation/Databricks-Customer-360-ebook-Final.pdf?raw=true') as pdf:
  ...
  doc = convert_pdf_to_markdown(pdf.content)

  print(doc)

# COMMAND ----------

# DBTITLE 1,Tokenize and Split PDF Text into Sentence Chunks
...
# MAGIC CREATE TABLE IF NOT EXISTS pdf_invoices_embeddings (
...
# DBTITLE 1,Stream Invoices and Documentation with Embeddings
(spark.readStream.table('invoices')
  ...
  .writeStream
    .trigger(availableNow=True)
    .option("checkpointLocation", f'dbfs:{volume_folder}/checkpoints/invoices_chunk')
    .table('pdf_invoices_embeddings').awaitTermination())
...
# MAGIC %sql
# MAGIC select count(*) from pdf_invoices_embeddings;
...
source_table_fullname = f"{catalog}.{dbName}.pdf_invoices_embeddings"
...
question = "Qual o número da Inovice ?"
